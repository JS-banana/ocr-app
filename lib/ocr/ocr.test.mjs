...
describe("parseModelCatalog", () => {
  it("accepts the checked-in 3-model manifest and derives downloadBytes", () => {
    ...
    // Small/Medium 共用 full 字典、boxThresh 0.45、dictSize 18710
    for (const m of [catalog.models[1], catalog.models[2]]) {
      ...
      assert.equal(m.files.dict.url, "/models/ppocr_keys_v6_full.json");
    }
...
describe("catalog revision cache keys", () => {
  it("builds whole-catalog revision keys covering shared dict once", () => {
    const catalog = parseModelCatalog(manifestFixture());
    const keys = catalogCacheKeys(catalog);
    // tiny det/rec/dict + small det/rec/dict + medium det/rec（dict 与 small 同 key）
